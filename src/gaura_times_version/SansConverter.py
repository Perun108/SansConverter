...
from src.about import Ui_Dialog2
...
class SansConverter(QtWidgets.QMainWindow):
    ...
    def pressed(self):
        ...
        enc_names = {"Balaram": balaram, "IAST": iast, "HK": hk, "Velthius": velthius}
        ext_enc_names = {
            "Balaram": balaram_ext,
            "IAST": iast_ext,
            "HK": hk_ext,
        ...
        if encoding1 == encoding2:  # To save time for identical encodings
            ...
            if encoding1 not in ext and text.islower():
                self.convert(text, enc_names[encoding1], hk, encoding1, encoding2)
            else:
                self.convert(text.lower(), ext_enc_names[encoding1], hk_ext, encoding1, encoding2)
        # Simplify transliteration of the similar encodings that are based on Roman script
            ...
            self.convert(text, enc_names[encoding1], enc_names[encoding2], encoding1, encoding2)
        # For transliterating between Roman and Cyrillic transliterations
        else:
            self.convert(text, ext_enc_names[encoding1], ext_enc_names[encoding2], encoding1, encoding2)
